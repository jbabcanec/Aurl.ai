...
# Gradient anomaly detection disabled - issues have been fixed
...
from src.training.utils.checkpoint_manager import CheckpointManager, CheckpointConfig
...
def create_dataset(config: Dict[str, Any]) -> LazyMidiDataset:
    ...
    dataset = LazyMidiDataset(
        ...
        augmentation_probability=data_config.get("augmentation", {}).get("probability", 0.5)
    )
...
def run_training(config: Dict[str, Any], model: MusicTransformerVAEGAN, 
    ...
    checkpoint_config = CheckpointConfig(
        ...
        save_frequency=config["training"].get("save_every_n_epochs", 5)
    )
    ...
    loss_framework = ComprehensiveLossFramework(
        ...
        warmup_epochs=config["training"].get("warmup_steps", 50) // 10  # Convert steps to epochs estimate
    ).to(device)
    
    # Create trainer config with mapped parameters
    ...
    trainer_config = TrainingConfig(
        ...
        min_learning_rate=training_config.get("min_learning_rate", 1e-7)
    )
